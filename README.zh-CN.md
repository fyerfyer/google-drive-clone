...
- **自动向量嵌入流水线**：基于事件驱动的高效后台 Embedding 任务，支持自动创建、内容变更同步及删除清理。
- **语义检索**：基于 Qdrant 向量数据库，支持自然语言文档搜索，由自动流水线驱动。
- **协作文档编辑**：集成 OnlyOffice，支持实时在线文档编辑。
- **弹性对象存储**：基于 MinIO 的 S3 兼容存储，轻松横向扩展。
- **去重与状态同步**：基于 Hash 的重复校验，确保相同内容不重复计算，同时实时同步索引状态。
...
### 自动化知识流水线 (Embedding Workflow)

MindDrive 拥有完整的、事件驱动的自动化 Embedding 流水线，确保向量库与文件内容完全同步。

- **自动触发索引**：文件创建或更新后，通过内部事件总线立即触发异步索引任务。
- **状态全生命周期追踪**：数据库记录 `pending`、`processing`、`completed`、`failed` 等状态，前端实时展示“AI 可用”标识。
- **智能 Hash 去重**：通过文件 Hash 校验，避免对重复内容进行冗余 Embedding 计算，节省 Token 和计算开销。
- **级联清理机制**：利用 Mongoose 钩子，确保文件物理删除时，Qdrant 向量库中的对应记录被精准清除。
- **异步 Worker 执行**：整个流程由独立的 `EmbeddingWorker` 后台执行，不阻塞用户主操作流程。
